...
const splitSon = grandson.split(" ");
const nameLength = splitSon.length;
...
console.log(altRick.length);

// Q4
// How many characters are there in rickSaying (without spaces)?
let count = 0;
let sayingSplit = rickSaying.split(" ");

for (let i = 0; i < rickSaying.length; i++) {
  if (rickSaying[i] != " ") count++;
}
console.log(count);
...
let mortyName = "";
mortyName = grandson.replace("Antoine ", "");
...
newEnemies = newEnemies.replace("Scary Terry, ", "");
